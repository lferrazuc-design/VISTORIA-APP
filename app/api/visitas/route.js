...
export async function GET(request) {
  try {
    ...
    let result;
    if (codigoFazenda) {
      result = await sql`SELECT * FROM visitas WHERE codigo_fazenda = ${codigoFazenda} ORDER BY timestamp DESC`;
    } else if (fornecedor) {
      result = await sql`SELECT * FROM visitas WHERE fornecedor = ${fornecedor} ORDER BY timestamp DESC`;
    } else {
      result = await sql`SELECT * FROM visitas ORDER BY timestamp DESC LIMIT 500`;
    }
